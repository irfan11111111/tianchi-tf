...
flags.DEFINE_string("outputs", default="./output_dir", help='')
...
flags.DEFINE_string("modelZooBasePath", default=os.path.join("./model", "eztransfer_modelzoo"), help="eztransfer_modelzoo")
...
class Config(object):
    def __init__(self, mode, config_json):
        ...
        if not FLAGS.usePAI:
            FLAGS.modelZooBasePath = config_json.get("modelZooBasePath", os.path.join("./model", "eztransfer_modelzoo"))
        ...
        if self.mode == 'train' or self.mode == "train_and_evaluate" \
            ...
            self.enable_auto_mixed_precision=False
            self.distribution_strategy =False
            self.num_accumulated_batches=1
            self.num_model_replica=1
...
class EzTransEstimator(object):
    ...
    def run_preprocess(self, reader, writer):
        print(1111111111,reader.get_input_fn())
        for batch_idx, outputs in enumerate(self.estimator.predict(input_fn=reader.get_input_fn(),
